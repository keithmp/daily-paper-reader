---
title: Joint Geometric-Chemical Distance for Protein Surfaces
title_zh: 蛋白质表面的几何-化学联合距离
authors: "Himanshu Swami, John M. McBride, Jean-Pierre Eckmann, Tsvi Tlusty"
date: 2026-03-10
pdf: "https://arxiv.org/pdf/2603.09860v1"
tags: ["query:q3"]
score: 6.0
evidence: 比较蛋白质表面以区分构象变化与结构演化差异
tldr: 蛋白质功能由其表面的形状和化学性质共同决定，但现有方法往往将两者分离。本研究提出IFACE框架，通过将内在几何与空间分布的化学场进行概率耦合来对齐蛋白质表面，并推导出一种集成的几何-化学距离。该方法能有效区分构象变化与结构差异，在细胞色素P450家族中成功识别出保守的催化口袋，为比较蛋白质界面和检测功能相关补丁提供了新准则。
selection_source: fresh_fetch
motivation: 现有的蛋白质比较方法通常将几何形状与化学特性分开处理，忽略了两者在分子表面上的耦合组织关系。
method: 引入IFACE框架，通过概率耦合内在几何与化学场来实现表面对齐，并定义了一个统一的几何-化学距离度量。
result: 该方法在区分结构发散与构象变异方面优于传统方法，并能揭示复杂蛋白质家族中的保守功能特征。
conclusion: IFACE为蛋白质界面的定量比较和功能性相互作用区域的检测建立了一个科学且可解释的基础。
---

## 摘要
蛋白质功能在分子表面执行，其形状和化学特性共同决定了相互作用。然而，大多数比较方法将这些方面分开处理，要么侧重于全局折叠，要么侧重于局部描述符，从而忽略了它们的耦合组织。在这里，我们介绍了 IFACE（内在场对齐耦合嵌入），这是一个基于对应关系的框架，通过将内在几何与空间分布的化学场进行概率耦合来对齐蛋白质表面。通过这种对齐，我们推导出一个几何-化学联合距离，该距离在单一公式中整合了结构和物理化学差异。在多种蛋白质中，与基于折叠的相似性度量相比，这种距离能更有效地将构象变异与真正的结构差异区分开来。应用于细胞色素 P450 家族，它揭示了连贯的家族级组织，并识别出尽管拓扑结构复杂但仍保守的埋藏催化口袋。通过将可解释的表面对应关系与统一距离联系起来，IFACE 为比较蛋白质界面和检测跨蛋白质的功能相关相互作用斑块建立了原则性基础。

## 速览
**TLDR**：蛋白质功能由其表面的几何形状和化学特性共同决定，但现有方法往往将两者分离。本研究提出IFACE框架，通过概率耦合内在几何与空间分布的化学场来对齐蛋白质表面，并推导出一种联合几何-化学距离。该方法能有效区分构象变化与结构差异，在细胞色素P450家族中成功识别出保守的催化口袋，为比较蛋白质界面和检测功能相关补丁提供了新工具。 \
**Motivation**：现有的蛋白质比较方法通常将几何形状与化学特性分开处理，忽略了它们在分子表面上的耦合组织。 \
**Method**：提出IFACE框架，利用概率耦合将内在几何与空间化学场对齐，从而定义统一的几何-化学距离。 \
**Result**：该距离在区分构象变异与结构分歧方面优于传统的基于折叠的相似性度量，并能识别复杂的保守催化口袋。 \
**Conclusion**：IFACE为蛋白质界面的比较和功能相关相互作用区域的检测建立了一个基于原理的统一框架。

---

## Abstract
Protein function is executed at the molecular surface, where shape and chemistry act together to govern interaction. Yet most comparison methods treat these aspects separately, privileging either global fold or local descriptors and missing their coupled organization. Here we introduce IFACE (Intrinsic Field-Aligned Coupled Embedding), a correspondence-based framework that aligns protein surfaces through probabilistic coupling of intrinsic geometry with spatially distributed chemical fields. From this alignment, we derive a joint geometric--chemical distance that integrates structural and physicochemical discrepancies within a single formulation. Across diverse proteins, this distance separates conformational variability from true structural divergence more effectively than fold-based similarity measures. Applied to the cytochrome P450 family, it reveals coherent family-level organization and identifies conserved buried catalytic pockets despite the complex topology. By linking interpretable surface correspondences with a unified distance, IFACE establishes a principled basis for comparing protein interfaces and detecting functionally related interaction patches across proteins.