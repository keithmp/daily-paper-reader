...
title_zh: NEPMaker：针对大超胞的神经演化机器学习势主动学习
...
## 摘要
机器学习势 (MLPs) 具有接近第一性原理的精度，但在处理训练分布之外的原子环境时往往会失效。主动学习可以缓解这一局限性；然而，由于对整个构型进行标注的成本极高，其在大规模模拟中的应用受到了阻碍。在此，我们开发了一个名为 NEPMaker 的 D-最优性驱动主动学习框架，用于 GPUMD 软件包中实现的神经演化势 (NEP)。该框架能够实时识别外推原子环境并将其嵌入到局部周期性结构中，通过优化边界原子使其保持接近训练分布。这一策略使大规模模拟能够直接贡献于数据集构建，在显著减少外推误差的同时，提高了模型的鲁棒性和迁移性。该框架为在复杂材料体系（包括涉及缺陷、界面和相变的体系）中构建可靠的机器学习势提供了一条可扩展的途径。
