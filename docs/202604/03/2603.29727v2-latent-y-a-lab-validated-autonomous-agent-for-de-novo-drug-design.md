...
tldr: Latent-Y是一个自主药物设计AI智能体，能根据文本提示完成从文献调研到候选抗体设计的全流程。它集成了Latent-X2生成模型及多种生物信息学工具，在9个靶点的实验中成功设计出6个具有高亲和力的纳米抗体，且设计速度比人类专家快56倍，显著提升了抗体药物研发的效率和自动化水平。
selection_source: fresh_fetch
motivation: 针对传统药物研发流程中专家工作流迭代缓慢、难以并行且扩展性差的挑战，旨在通过AI智能体实现自动化的从头药物设计。
method: 开发了集成Latent-X2生成模型和生物信息学工具的AI智能体Latent-Y，可自主执行文献分析、靶点识别及抗体序列生成与验证。
result: "在九个靶点的测试中，Latent-Y在无人工干预下实现了67%的实验成功率，部分抗体亲和力达纳摩尔级别，且研发耗时缩短了56倍。"
conclusion: Latent-Y证明了AI智能体在自主完成复杂生物药物设计任务方面的巨大潜力，为加速多种治疗模式的药物发现提供了通用框架。
---

## 摘要
药物研发依赖于迭代的专家工作流，这些工作流难以并行化且难以扩展。在此，我们介绍了 Latent-Y，这是一种能够根据文本提示自主执行完整抗体设计任务的 AI 智能体，涵盖了文献综述、靶点分析、表位识别、候选药物设计、计算验证以及可用于实验室的序列筛选。Latent-Y 集成在 Latent Labs 平台中，它与药物研发专家在相同的环境中运行，并可访问生物信息学工具、生物数据库和科学文献。该智能体可以完全端到端地自主运行，也可以协作运行，由研究人员审查进度、提供反馈并指导后续步骤。候选抗体是使用 Latent-X2 生成的，这是我们用于类药抗体设计的尖端生成模型。我们展示了该智能体在三种不同任务类型中的能力：由治疗规范引导的表位发现、跨物种结合剂设计，以及基于针对人类转铁蛋白受体以穿过血脑屏障的科学出版物进行的自主设计。在九个靶点中，Latent-Y 针对其中六个产生了经实验室证实的纳米抗体结合剂，在无需人工筛选或干预的情况下，实现了 67% 的靶点级成功率，且结合亲和力达到了个位数纳摩尔量级。在用户研究中，与 Latent-Y 合作的专家完成设计任务的速度比独立专家的预估时间快 56 倍，将数周的工作压缩至数小时。由于 Latent-X2 是一种用于生物制品设计的通用原子级模型，相同的智能体架构可以自然地扩展到大环肽和微型结合剂设计任务中，从而拓宽了跨治疗模式的自主发现。Latent-Y 已向选定的合作伙伴开放，网址为 https://platform.latentlabs.com。
