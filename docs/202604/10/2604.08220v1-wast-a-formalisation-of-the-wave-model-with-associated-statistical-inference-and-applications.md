...
date: 2026-04-09
pdf: "https://arxiv.org/pdf/2604.08220v1"
tags: ["query:q5"]
score: 7.0
...
## 摘要
我们提出了最初在历史语言学中开发、但在人文科学中具有进一步应用的“波浪模型”的数学形式化。该模型假设新特征在种群中出现，并根据亲疏程度传播到邻近种群。它主要用于描述密切相关种群（例如几种方言）的共同演化。这种长期接触的情况无法通过基于树状结构的竞争模型得到准确表示。我们构建了一个全贝叶斯生成模型，其中创新沿固定图传播，并根据死亡过程消失。随后，我们开发了一种 Gibbs 采样器中的 Metropolis-Hastings 算法，用于从图上的后验分布中进行采样。我们在模拟数据集以及多个真实数据集上测试了我们的方法。
