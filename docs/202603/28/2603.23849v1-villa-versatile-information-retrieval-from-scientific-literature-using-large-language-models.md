...
tags: ["query:q7"]
score: 6.0
evidence: 利用AI从文献中提取科学信息
tldr: 针对科学文献信息提取（SIE）中高质量数据集匮乏及现有方法难以处理复杂开放式任务的问题，本文提出了VILLA框架。该框架采用多步检索增强生成（RAG）技术，专注于病毒学领域，特别是提取影响宿主相互作用的流感病毒突变。研究还构建了一个包含629个突变的基准数据集，并证明了VILLA在处理复杂科学文本方面优于现有SOTA工具。
selection_source: fresh_fetch
motivation: 现有的科学信息提取方法多局限于简单任务和特定领域，缺乏处理复杂、开放式科学问题的能力及高质量基准数据。
method: 开发了名为VILLA的多步检索增强生成（RAG）框架，并针对病毒突变提取任务构建了包含239篇论文、629个突变位点的新型数据集。
result: 实验结果表明，VILLA在病毒突变提取任务上的表现显著优于传统RAG及其他最先进的代理类SIE工具。
conclusion: VILLA框架为自动化构建高质量科学数据集提供了有效途径，展示了LLM在处理复杂、跨学科科学文献提取任务中的巨大潜力。
---

## 摘要
缺乏高质量的基准数据集来训练机器学习（ML）模型，阻碍了人工智能（AI）在科学研究中的潜力。利用大语言模型（LLMs）从文献中进行科学信息提取（SIE）正成为一种自动化创建这些数据集的强大方法。然而，现有的基于 LLM 的 SIE 方法和基准研究主要集中在生物医学和化学等广泛领域，且局限于基于选择的任务，并侧重于从简短且格式良好的文本中提取信息。SIE 方法在复杂、开放式任务中的潜力仍有待深入探索。在本研究中，我们利用了一个在 SIE 中几乎被忽视的领域——病毒学，来填补这些研究空白。我们设计了一个独特的开放式 SIE 任务，即提取特定病毒中改变其与宿主相互作用的突变。我们开发了一个名为 VILLA 的新型多步检索增强生成（RAG）框架，用于 SIE。与此同时，我们从 239 篇科学出版物中整理了一个包含 10 种甲型流感病毒蛋白中 629 个突变的新型数据集，作为突变提取任务的基准。最后，通过一种新颖且全面的评估，并与基础 RAG 以及其他最先进的基于 RAG 和智能体的 SIE 工具进行比较，我们证明了 VILLA 的卓越性能。
