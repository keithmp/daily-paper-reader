...
## 摘要
线性化字符串表示是可扩展自回归分子生成的基础；然而，它们引入了一种根本性的模态失配，即单个分子图会映射到多个不同的序列。这种歧义导致了“轨迹分叉”（trajectory divergence），即由于线性化历史的不同，结构等价的部分图的潜在表示会发生偏移。为了在不放弃高效字符串表述的情况下解决这一问题，我们提出了结构不变生成式分子对齐（SIGMA）。SIGMA 并非改变线性表示，而是通过令牌级对比目标使模型能够严格识别几何对称性，该目标显式地对齐了共享相同后缀的前缀的潜在状态。此外，我们引入了同构束搜索（IsoBeam），通过动态剪枝等效路径来消除推理过程中的同构冗余。在标准基准上的实证评估表明，SIGMA 弥合了序列可扩展性与图保真度之间的差距，与强基线模型相比，在多参数优化中实现了卓越的样本效率和结构多样性。
