...
tags: ["query:q7"]
score: 8.0
evidence: 用于单细胞基因表示和生物知识编码的AI基础模型
...
motivation: 旨在揭示单细胞大模型学习到的高维基因表示中究竟蕴含了哪些具体的生物学知识及其组织形式。
method: 采用自动化假设筛选方法，通过63次迭代系统解码了scGPT内部表示的谱几何结构。
result: 发现模型通过不同维度精确编码了亚细胞定位、蛋白质相互作用强度及转录调控逻辑，且其层级演化反映了细胞分泌路径。
conclusion: 研究证明生物Transformer模型能够学习到高度可解释的细胞组织内部模型，对模型审计和生物医学应用具有重要意义。
---
