...
title_zh: 因果细胞上下文迁移学习 (C3TL)：一种用于预测未知扰动效应的高效架构
...
## 摘要
预测化学和遗传扰动对定量细胞状态的影响是计算生物学、分子医学和药物研发中的核心挑战。近期的研究利用大规模单细胞数据和巨型基础模型来解决这一任务。然而，此类计算资源和广泛的数据集在学术或临床环境中并不总是可获得的，从而限制了其实用性。在此，我们提出了一种用于扰动效应预测的轻量级框架，该框架利用了生物干预的结构化特性以及特定的归纳偏置/不变性。我们的方法利用有关扰动效应的现有信息，实现了向新上下文的泛化，且仅需要广泛可得的大宗（bulk）分子数据。通过将特定上下文扰动效应的预测结果与真实的大规模干预实验进行对比，广泛的测试证明了在新上下文中预测的准确性。所提出的方法与最先进的（SOTA）基础模型相比具有竞争力，但需要更简单的数据、更小的模型规模和更少的时间。通过专注于稳健的大宗信号和高效架构，我们展示了在无需专用硬件或超大型模型的情况下，准确预测扰动效应是可能的，从而为在生物医学领域广泛利用因果学习方法开辟了道路。
