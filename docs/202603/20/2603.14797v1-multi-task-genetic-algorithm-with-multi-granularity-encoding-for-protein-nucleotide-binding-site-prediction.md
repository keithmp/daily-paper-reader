...
title_zh: 结合多粒度编码的多任务遗传算法用于蛋白质-核苷酸结合位点预测
...
tldr: 准确识别蛋白质-核苷酸结合位点对药物研发至关重要。针对现有方法特征表示不足和融合机制僵化的问题，本文提出MTGA-MGE框架。该框架结合多粒度编码网络提取判别性信号，利用遗传算法自适应进化任务特定的融合策略，并引入外部邻域机制促进跨任务协作学习。实验证明，该方法在15个数据集上均达到SOTA水平，在低资源场景下也表现稳健，为蛋白质-配体相互作用研究提供了新方案。
selection_source: fresh_fetch
motivation: 现有计算方法在处理高维冗余生物数据时存在特征表示不充分和跨任务信息协同效率低的问题。
method: 提出MTGA-MGE框架，通过多粒度编码、遗传算法优化融合策略及外部邻域机制实现高效的结合位点预测。
result: 在15个核苷酸数据集上的广泛评估显示，该方法在丰富资源和稀缺资源场景下均优于现有最先进技术。
conclusion: MTGA-MGE为解码复杂的蛋白质-配体相互作用提供了一种高自适应性的方案，具有重要的生物医学应用潜力。
---

## 摘要
准确识别蛋白质-核苷酸结合位点是揭示分子机制和加速药物研发的基础。然而，由于特征表示不足和融合机制僵化，现有的计算方法往往表现欠佳，阻碍了对跨任务信息协同效应的有效利用。为填补这一空白，我们提出了 MTGA-MGE 框架，该框架集成了多任务遗传算法与多粒度编码，以增强结合位点预测。具体而言，我们开发了一个多粒度编码（MGE）网络，该网络协同多尺度卷积和自注意力机制，从高维、冗余的生物数据中提取判别性信号。为了克服静态融合的限制，采用遗传算法自适应地演化特定任务的融合策略，从而有效提高模型的泛化能力。此外，为了促进协同学习，我们引入了外部邻域机制（ENM），利用生物相似性来促进跨任务的有针对性的信息交换。在 15 个核苷酸数据集上的广泛评估表明，MTGA-MGE 不仅在数据丰富的高资源场景中达到了新的最先进水平，而且在稀有的低资源环境下也保持了强大的竞争优势，为后基因组时代解码复杂的蛋白质-配体相互作用提供了一种高度自适应的方案。
