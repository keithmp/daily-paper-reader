...
tldr: 植硅体分析对重建过去植被和人类活动至关重要，但传统人工镜检效率低下。本研究开发了Sorometry，一个端到端的AI流水线，通过处理显微镜z轴堆叠扫描生成2D图像和3D点云。利用ConvNeXt和PointNet++的多模态融合模型，实现了高精度的植硅体自动分类与分割，并结合贝叶斯模型预测植物来源，将植硅体研究提升至“组学”规模。
selection_source: fresh_fetch
motivation: 传统的植硅体人工镜检方法费时费力且主观性强，严重限制了大规模考古和古生态研究的效率与标准化。
method: 开发了Sorometry流水线，利用多模态融合模型结合2D图像与3D点云数据进行分类，并采用贝叶斯有限混合模型预测植物贡献。
result: "模型在24种形态分类中达到77.9%的准确率，且3D数据的引入显著提升了对复杂形态植硅体的识别能力。"
conclusion: 该研究通过AI技术实现了植硅体的高通量、标准化分析，为考古学和古生态学提供了具有可重复性的群体水平表征工具。
---

## 摘要
植硅体分析是重建过去植被和人类活动的关键工具，但传统方法受限于耗时耗力的手工显微镜观察。为解决这一瓶颈，我们提出了 Sorometry：一个用于植硅体高通量数字化、推理和解释的综合端到端人工智能流水线。我们的工作流处理 z-轴堆叠的光学显微镜扫描图像，自动生成单个微观颗粒的同步 2D 正射影像和 3D 点云。我们开发了一个多模态融合模型，结合了用于 2D 图像分析的 ConvNeXt 和用于 3D 点云分析的 PointNet++，并辅以图形用户界面供专家进行标注和审核。在来自玻利维亚亚马逊地区的参考藏品和考古样本上的测试表明，我们的融合模型在 24 种诊断性形态类型中实现了 77.9% 的全局分类准确率，分割质量达到 84.5%。至关重要的是，3D 数据的集成被证明对于区分复杂的形态类型（如禾本科硅质短细胞植硅体）至关重要，因为这些类型的诊断特征在 2D 投影中往往因其朝向而被遮挡。除了单个物体的分类，Sorometry 还结合了贝叶斯有限混合模型，以在组合层面预测整体植物来源贡献，成功识别了复杂混合样本中的玉米和棕榈等特定植物。这一集成平台将植硅体研究转变为“组学”规模的学科，极大地扩展了分析能力，标准化了专家判断，并实现了考古和古生态组合的可重复、群体水平的特征描述。
