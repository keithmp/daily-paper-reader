...
motivation: 旨在解决动态生物反应器模拟中重复求解约束性代谢模型所导致的计算效率低下和数值不可行性问题。
method: 利用神经网络代理模型替代简约通量平衡分析中的嵌入式优化问题，以实现代谢通量的平滑近似并消除重复的线性规划求解。
result: 在大肠杆菌补料分批发酵的案例中，该代理模型在底物受限的极端条件下仍能提供稳定的代谢通量预测，优于传统的线性规划方法。
conclusion: 该框架为基因组规模代谢模型在复杂生物反应器环境下的动态模拟提供了一种高效、连续且数值稳定的计算方案。
---

## 摘要
生物系统受细胞内代谢与生物反应器运行之间跨越多个时间尺度的耦合相互作用支配。基于约束的代谢模型被广泛用于描述细胞内代谢，但在动态模型中每个时间步重复求解优化问题会引入与不可行性和计算效率相关的数值挑战。本研究提出了一种多尺度建模框架，将全基因组尺度、基于约束的代谢模型与动态生物反应器模拟相结合。细胞内代谢在简约通量平衡分析中通过正通量变量进行描述，由此产生的嵌入式优化问题被神经网络代理模型所取代。该代理模型提供了嵌入式优化映射的光滑近似，并消除了模拟过程中重复的线性规划求解。该方法在大肠杆菌的补料分批发酵中得到了验证，其中代理模型在底物受限条件下产生了细胞内通量，而底层的线性规划在此时通常是不可行的。该框架提供了细胞内代谢的连续表示，适用于生物反应器配置下全基因组尺度的动态模拟。
