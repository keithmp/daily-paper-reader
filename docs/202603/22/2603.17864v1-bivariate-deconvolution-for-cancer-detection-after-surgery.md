...
tags: ["query:q7"]
score: 7.0
evidence: 全基因组测量和基于突变的VAF用于癌症检测
tldr: 本研究针对癌症术后微小残留病灶（MRD）检测，提出了一种双变量反卷积模型。该模型利用患者手术前后的循环游离DNA（cfDNA）甲基化图谱，通过将观测值建模为肿瘤和背景信号的混合，有效估算术后肿瘤比例。这种方法结合了术前术后数据，在肿瘤信号微弱时仍能保持高灵敏度，为术后疾病复发监测和治疗决策提供了更精准的液体活检方案。
selection_source: fresh_fetch
motivation: 旨在解决术后微小残留病灶检测中，因肿瘤信号极度微弱而导致的估算不准确和灵敏度不足的问题。
method: 提出一种双变量反卷积模型，通过将术前与术后cfDNA甲基化数据联合建模为肿瘤与背景信号的混合，并利用离散化网格进行快速极大似然估计。
result: 该方法通过关联手术前后的测量值，在肿瘤信号极低的情况下显著提升了术后肿瘤比例的估算精度。
conclusion: 该模型在预测一年无复发生存率方面具有良好表现，为基于cfDNA甲基化的癌症术后监测提供了一种高效的计算方案。
---

## 摘要
癌症患者术后微小残留病灶（MRD）的检测可为疾病复发提供早期标志物，并指导后续治疗决策。通过液体活检，例如利用基于突变的变异等位基因频率（VAF）值测量循环肿瘤DNA（ctDNA），可以准确且灵敏地评估癌症术后的肿瘤负荷。然而，为了适用于所有患者，这要么需要基于肿瘤信息的患者特异性突变面板，要么需要灵敏的、与肿瘤无关的全基因组测量。我们提出了一种在全基因组筛选中考虑患者特异性特征的解决方案。为此，我们引入了一种双变量反卷积模型，用于从患者术前和术后的循环游离DNA（cfDNA）甲基化谱中估计肿瘤比例。观测值被建模为两个双变量潜变量的卷积，分别对应于肿瘤信号和背景信号，并由每次测量时的肿瘤比例进行混合。这种双变量方法将术前和术后的测量联系起来，从而改进了术后肿瘤比例的估计，尤其是在肿瘤信号可能非常微弱或缺失的情况下。我们通过将每个潜变量的双变量密度离散化为每对观测值的二维网格来近似卷积的似然性，从而实现快速的最大似然估计。我们在一年无复发生存期中，评估了基于cfDNA甲基化估计的术后肿瘤比例相对于现有基于突变的VAF值的预测性能。
