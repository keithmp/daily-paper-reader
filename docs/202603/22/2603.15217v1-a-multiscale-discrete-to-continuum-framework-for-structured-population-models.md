...
title_zh: 一种用于结构化种群模型的多尺度离散-连续框架
authors: "Eleonora Agostinelli, Keith L. Chambers, Helen M. Byrne, Mohit P. Dalwadi"
date: 2026-03-16
pdf: "https://arxiv.org/pdf/2603.15217v1"
tags: ["query:q5"]
score: 6.0
evidence: 结构化种群模型与性状变异
...
result: 成功推导出主体域的非线性平流方程及波前和停滞点附近的平流-扩散过程，并为不适用连续描述的区域提供了离散边界层描述。
conclusion: 该多尺度框架为离散结构化系统提供了具有渐近一致边界条件的连续动力学描述，增强了模型的可分析性和数值计算效率。
---

## 摘要
生物种群的数学模型通常使用离散结构类来捕捉个体间的性状差异（如年龄、大小、表型、细胞内状态）。将这些离散模型放大为连续描述可以提高分析的可处理性和数值解的可扩展性。然而，仅基于泰勒展开的常用放大方法可能会在截断阶数、一致有效性和边界条件方面引入歧义。为解决这一问题，本文引入了一种离散多尺度框架，用于系统地推导结构化种群模型的连续近似。通过将多尺度法和匹配渐近展开法应用于离散系统，我们识别了结构空间中适合连续表示的区域，并推导出了相应的偏微分方程。主阶动力学由主体区域内的非线性平流方程以及领先波前和驻点附近微小内层中的平流-扩散过程给出。我们进一步为连续表示根本不适用的区域推导了离散边界层描述。最后，我们在一个用于早期动脉粥样硬化的简单脂质结构模型上演示了该方法，并验证了离散描述与连续描述之间的一致性。我们提出的多尺度框架可应用于其他具有离散结构的异质系统，以获得具有渐近一致边界条件的适当放大动力学。
