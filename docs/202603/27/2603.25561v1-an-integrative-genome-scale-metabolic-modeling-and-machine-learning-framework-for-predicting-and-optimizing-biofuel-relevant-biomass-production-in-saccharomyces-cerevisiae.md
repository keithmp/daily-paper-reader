...
title_zh: 预测与优化酿酒酵母中生物燃料相关生物量生产的整合全基因组尺度代谢模型与机器学习框架
authors: "Neha K. Nair, Aaron D'Souza"
date: 2026-03-26
pdf: "https://arxiv.org/pdf/2603.25561v1"
tags: ["query:q7"]
score: 8.0
evidence: 利用机器学习进行酵母基因组规模代谢建模
...
method: 结合了基因组规模代谢模型、通量平衡分析、多种机器学习回归模型、变分自编码器及贝叶斯优化算法。
result: "机器学习模型预测精度极高（R2>0.999），且通过贝叶斯优化使生物量通量实现了12倍的显著增长。"
conclusion: 该集成框架证明了基因组模拟、可解释机器学习与生成式模型在推进酵母代谢工程方面的巨大潜力。
---

## 摘要
酿酒酵母（Saccharomyces cerevisiae）是工业生物技术的基石生物，因其遗传易操作性和强大的发酵能力而备受重视。在各种环境和遗传扰动下准确预测生物量通量仍然是理性菌株设计的重大挑战。我们提出了一个结合 Yeast9 全基因组尺度代谢模型、机器学习和优化的计算框架，用于预测、解释和增强生物量通量。通过改变葡萄糖、氧气和铵的摄取速率，通量平衡分析生成了 2,000 个通量谱。随机森林和 XGBoost 回归器分别实现了 0.99989 和 0.9990 的 R2。变分自编码器揭示了四个不同的代谢簇，SHAP 分析确定了糖酵解、三羧酸循环（TCA 循环）和脂质生物合成是关键的生物量决定因素。计算机模拟过表达实现了 0.979 gDW/hr 的生物量通量，而营养约束的贝叶斯优化使通量增加了 12 倍（从 0.0858 增加到 1.041 gDW/hr）。生成对抗网络提出了化学计量学上可行的新型通量配置。该框架展示了全基因组尺度模拟、可解释机器学习和生成模型如何推进酵母代谢工程。
