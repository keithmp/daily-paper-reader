...
motivation: 生物医学表格数据缺乏内在空间组织，限制了视觉深度学习模型捕获局部结构和高阶特征交互的能力。
method: 提出 Dynomap 框架，通过全可微分渲染机制学习任务优化的特征空间拓扑，将无序表格向量转化为特征图。
result: 在液体活检和帕金森病等数据集上，Dynomap 的预测准确率较现有方法提升显著，并能生成可解释的特征模式。
conclusion: Dynomap 成功桥接了表格数据与视觉深度学习，为分析高维生物医学数据提供了一种高效且通用的新策略。
---
