...
tldr: 该研究针对生物信息学领域论文描述与代码实现不一致的问题，提出了“论文-代码一致性检测”新任务。通过构建包含48个项目的BioCon基准数据集，实现了句子级算法描述与函数级代码的对齐。研究提出了一种跨模态检测框架，利用预训练模型捕捉语义关联，并引入加权Focal Loss解决样本不平衡问题，为科学软件的可重复性评估奠定了基础。
selection_source: fresh_fetch
motivation: 旨在解决生物信息学领域中论文方法描述与实际代码实现之间普遍存在的不一致性，以提高软件可靠性和科学可重复性。
method: 构建了首个生物信息学一致性检测基准数据集BioCon，并提出一种结合预训练模型与加权Focal Loss的跨模态语义对齐框架。
result: 实验结果显示，该框架在一致性检测任务中表现优异，准确率达到0.9056，F1分数达到0.8011。
conclusion: 本研究开辟了论文-代码一致性分析的新方向，为自动化可重复性评估和科学软件的跨模态理解提供了重要工具。
---

## 摘要
确保研究论文与其对应的软件实现之间的一致性是软件可靠性和科学可复现性的基础。然而，这一问题尚未得到充分探索，特别是在生物信息学领域，该领域论文中的方法论描述与其实际代码实现之间的差异非常普遍。为了填补这一空白，本文引入了一项新任务，即论文-代码一致性检测，并整理了 48 个生物信息学软件项目及其相关出版物。我们系统地将论文中的句子级算法描述与函数级代码片段进行对齐。结合专家标注和混合负采样策略，我们构建了生物信息学领域首个针对该任务定制的基准数据集，命名为 BioCon。基于该基准，我们进一步提出了一个跨模态一致性检测框架，旨在对自然语言描述与代码实现之间的语义关系进行建模。该框架采用统一的输入表示，并利用预训练模型捕捉论文与代码之间的深层语义对齐。为了减轻类别不平衡和困难样本的影响，我们引入了加权 Focal Loss 以增强模型的鲁棒性。实验结果表明，我们的框架能有效识别生物信息学中论文与代码的一致性，准确率达到 0.9056，F1 分数达到 0.8011。总的来说，本研究为论文-代码一致性分析开辟了新的研究方向，并为科学软件的自动化可复现性评估和跨模态理解奠定了基础。
