...
method: 建立了一个将参数快速随机梯度更新与超参数慢速选择-变异动力学相结合的双时间尺度交互智能体系统模型。
result: 证明了大种群极限下的分布演化规律，并推导出在强时间尺度分离下，超参数演化受参数动力学松弛后的有效适应度驱动。
conclusion: 该框架统一了种群学习与双层优化理论，阐明了噪声和多样性在训练中的作用，并证明利用有效适应度可优化种群更新。
---

## 摘要
基于群体的学习范式，包括进化策略、基于群体的训练（PBT）以及近期的模型合并方法，将快速的模型内优化与较慢的群体级自适应相结合。尽管这些方法在经验上取得了成功，但对其产生的集体训练动力学的通用数学描述仍不完整。我们提出了一个基于双时间尺度群体动力学的神经网络训练理论框架。我们将神经网络群体建模为一个相互作用的智能体系统，其中网络参数通过 SGD/Langevin 类型的快速噪声梯度更新进行演化，而超参数则通过较慢的选择-变异动力学进行演化。我们证明了参数和超参数联合分布的大群体极限，并在强时间尺度分离的情况下，推导出了超参数密度的选择-变异方程。对于每个固定的超参数，快速参数动力学弛豫到玻尔兹曼-吉布斯测度，从而为慢速演化诱导出一种有效适应度。这种平均动力学将基于群体的学习与双层优化以及经典的复制者-变异者模型联系起来，给出了群体均值向最适超参数移动的条件，并阐明了噪声和多样性在平衡优化与探索中的作用。数值实验展示了大群体机制和简化的双时间尺度动力学，并表明通过闭式解或群体级估计获取有效适应度可以改进群体级更新。
